...
#Pick what dataset to use and upload from Excel file
...
ggplot(data, aes(x = Year, y = Month, fill = `Mean Intensity (mg/l)`)) +
  geom_tile() +
  scale_fill_gradient(low = "red", high = "white", na.value = "lightgrey") +
...
ggplot(data, aes(x = Year, y = Month, fill = `Lowest DO (mg/l)`)) +
  geom_tile() +
  scale_fill_gradient(low = "red", high = "white",na.value = "lightgrey") +
  labs(x = "Year", y = "Month", title = "Lowest Dissolved Oxygen Value by Month")
